...
names[, nconst := as.integer(gsub("^nm", "", nconst))]
names = names[!is.na(primaryName)]
...
names[, primaryProfession := gsub("actress", "actor", primaryProfession)]
names[primaryProfession == "", primaryProfession := NA_character_]
names[, primaryProfession := strsplit(names[, primaryProfession], ",")]
names[, nProfessions := names[, primaryProfession] %>% lapply(length) %>% unlist()]
names[is.na(primaryProfession), nProfessions := NaN]
names[knownForTitles == "\\N", knownForTitles := NA_character_]
names[, knownForTitles := gsub("tt", "", knownForTitles)]
names[, knownForTitles := strsplit(knownForTitles, ",") %>% lapply(as.integer)]
...
# ggsave("plots/01_preprocessing_birthYear_histogram.jpg", height = 6, width = 12)
# ggplot(names[deathYear > 1872, ], aes(x = deathYear)) + geom_histogram(bins = 150)
# ggsave("plots/01_preprocessing_deathYear_histogram.jpg", height = 6, width = 12)
# ggplot(names[!is.na(lifespan)], aes(x = lifespan)) + geom_histogram(bins = 123) + scale_x_continuous(breaks = seq(0, 122, 2))
# ggsave("plots/01_preprocessing_lifespan_histogram.jpg", height = 6, width = 12)
# 
# 
# # PRIMARYPROFESSION
...
fwrite(names, paste0(path, "/name.basics_cleaned.csv"))
