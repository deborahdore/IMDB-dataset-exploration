...
#source("05_dashboard/dashboard_prepare.R")  # comment this line after running it once
...
ui <- fluidPage(
  ...
  dashboardPage(
    ...
    dashboardBody(
      tabItems(
        tabItem(tabName = "hexbins",
          fluidRow(
            box(width = 12, solidHeader = TRUE, status = "primary",
                column(width = 6,
                       selectInput(inputId = "varx", choices = c("Rating", "Votes", "Translations", "Runtime", "Startyear"),
                                   selected = "Rating", label = "Variable for x axis"),
                       selectInput(inputId = "vary", choices = c("Rating", "Votes", "Translations", "Runtime", "Startyear"),
                                   selected = "Votes", label = "Variable for y axis")
                       ),
                column(width = 6, plotOutput("hexbins"))
                )
          )
        ),
...
server <- function(input, output) {
  ...
  output$menu = renderMenu({
    sidebarMenu(
      menuItem("Correlations", tabName = "hexbins"),
  ...
  output$hexbins = renderPlot(plot_list[[paste0("hexbins_", input$varx, "_", input$vary)]])
