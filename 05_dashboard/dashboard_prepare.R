...
# hexbins ----
series = fread(paste0(path, "/dataset/merged_series_withNA.csv"))
v = c("averageRating", "numVotes", "nTranslations", "runtimeMinutes", "startYear")
v_label = c("Average Rating", "log(Number of Votes)", "Number of Translations", "Runtime in Minutes", "Start Year")
v_name = c("Rating", "Votes", "Translations", "Runtime", "Startyear")
for (i in seq_along(v)) {
  for (j in seq_along(v)) {
    if (i == j) next
    varx = v[[i]]
    vary = v[[j]]
    this_trans_x = ifelse(varx == "numVotes", "log10", "identity")
    this_trans_y = ifelse(vary == "numVotes", "log10", "identity")
    plot_list[[paste0("hexbins_", v_name[[i]], "_", v_name[[j]])]] =
      ggplot(series[if (varx == "runtimeMinutes" | vary == "runtimeMinutes") (runtimeMinutes <= 100) else TRUE],
             aes_string(x = varx, y = vary)) +
      geom_hex(bins = if (varx == "startYear" | vary == "startYear") 10 else 30) +
      scale_x_continuous(trans = this_trans_x, breaks = if (varx == "startYear") 2010:2022 else waiver()) +
      scale_y_continuous(trans = this_trans_y) +
      xlab(v_label[[i]]) + ylab(v_label[[j]])
  }
}
...
# genresBar ----
tb = fread(paste0(path, "/dataset/merged_series_withNA.csv"))
...
# genresViolin ---- 
tb2 = fread(paste0(path, "/dataset/merged_series_withNA.csv"))
